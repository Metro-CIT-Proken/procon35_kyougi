...
def make_nuki(amount_created):
    answer = []
    for j in amount_created:
        for _ in range(amount_created[j]):
            x = int(j)
            y = int(j)
...
def output(args, field=None):
    # fsize=fieldの大きさ
    fsize_x = args.width  # フィールドの固定サイズ
    fsize_y = args.height
    if field is None:
        fsize_x, fsize_y, field = make_field(args)

    # create =抜き型の数
    goal_field = field_to_string(field)
    create = {"16":10,"32":10,"64":5}
    ...
    output = {
        "board": {
            "width": fsize_x,
            "height": fsize_y,
            "start": goal_field,  # ぐちゃぐちゃにしたフィールド
            "goal":  field_to_string(start_field)  # 最初に作成したフィールド
        },
        "general": {
            "n": len(k_dict),
            "patterns": [{
...
class get:
    def __init__(self, seed=0, output="f.json", width=32, height=32, swap=100):
        self.output = output
        self.seed = seed
        self.width = width
        self.height = height
...
def do(args, field = None):
    output_data = output(args, field)
    if args.output:
